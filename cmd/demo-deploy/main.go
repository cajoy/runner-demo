...
func simulate() error {
	...
	if err != nil {
		return fmt.Errorf("verified build artifact is unavailable: %w", err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	page, err := exec.CommandContext(ctx, binary, "-export").Output()
	if err != nil {
		return fmt.Errorf("verified build artifact could not render its page: %w", err)
	}
	if err := os.WriteFile(".local-ci-out/preview.html", page, 0600); err != nil {
		return err
	}
