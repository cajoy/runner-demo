...
The CI orchestrator tests also verify that failed planning or finalization prevents the deployment simulation, and that all-reused plans still require final verification. Installation checks the pinned binary digest before replacing an existing binary.
